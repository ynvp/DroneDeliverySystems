...
class App(customtkinter.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.calculate_path_button.grid(pady=(20, 0), padx=(20, 20), row=0, column=0)

        self.clear_marker_button = customtkinter.CTkButton(
            master=self.frame_left,
            text="Clear locations",
            command=self.clear_marker_list,
        )
        self.clear_marker_button.grid(pady=(20, 0), padx=(20, 20), row=1, column=0)
        ...
        self.marker_list_box.grid(pady=(15, 0), padx=(15, 15), row=2, column=0)
        ...
        self.button_5 = customtkinter.CTkButton(
            master=self.frame_right, text="Search", width=90, command=self.search
        )
        ...
        self.map_widget.add_right_click_menu_command(
            label="Add Charging Point",
            command=self.add_charging_point_marker_event,
            pass_coords=True,
        )
        self.map_widget.add_right_click_menu_command(
            label="Delete last added location",
            command=self.delete_last_marker,
            pass_coords=True,
        )
        self.map_widget.add_left_click_map_command(self.add_charging_point_marker_event)
        ...
        self.marker_list_box.insert(tkinter.END, "Left click to add charging point.")
        self.marker_list_box.insert(
            tkinter.END,
            "Right click to add warehouse/customer/delete last added location.",
        )
        # load images
        self.current_path = os.path.join(os.path.dirname(os.path.abspath(__file__)))
        self.warehouse_image = ImageTk.PhotoImage(
            Image.open(
                os.path.join(self.current_path, "images", "warehouse.png")
            ).resize((40, 40))
        )
        self.package_image = ImageTk.PhotoImage(
            Image.open(os.path.join(self.current_path, "images", "package.png")).resize(
                (40, 40)
            )
        )
        self.cp_image = ImageTk.PhotoImage(
            Image.open(os.path.join(self.current_path, "images", "cp.png")).resize(
                (40, 40)
            )
        )
    ...
    def run(self):
        if (
            len(r.warehouses) > 0
            and len(r.package_weights) > 0
            and len(r.charging_points) > 0
        ):
            ...
            marker_list_warehouses = dict(
                [
                    (warehouse.text.split("\n")[0], warehouse)
                    for warehouse in self.warehouse_marker_list
                ]
            )
            ...
            if current_loc == None:
                tkinter.messagebox.showerror("error", "No current location set!.")
            ...
            if self.final_path != None:
                ...
                print("Log: Route was not found due to insufficient charging points!")
                tkinter.messagebox.showerror(
                    "error",
                    "Charging points are not found enroute. Kindly add charging points.",
                )
                self.marker_list_box.insert(
                    tkinter.END,
                    "Charging points are not found enroute. Kindly add charging points",
            ...
            if len(r.warehouses) == 0:
                tkinter.messagebox.showerror("error", "Warehouses not yet added!")
                print("Log: warehouses not yet added")
            if len(r.package_weights) == 0:
                tkinter.messagebox.showerror("error", "Packages not yet added!")
                print("Log: Packages not yet added")
            if len(r.charging_points) == 0:
                tkinter.messagebox.showwarning(
                    "warning",
                    "Charging points were not added. This may affect path. Exiting calculation.",
                )
    ...
    def add_customer_marker_event(self, coords):
        ...
        new_marker = self.map_widget.set_marker(
            ...
            + "\n Package weight: "
            + self.inputDialog.weight,
            icon=self.package_image,
            text_color="blue",
            command=self.assign_weight(
                "Customer" + str(self.current_customer_counter), self.inputDialog.weight
            ),
        )
        # r.delivery_locations.append(
        #     ["Customer"+str(self.current_customer_counter), (coords[0], coords[1])])
        r.delivery_locations["Customer" + str(self.current_customer_counter)] = (
            coords[0],
            coords[1],
        )
        self.marker_list_box.insert(
    ...
    def add_warehouse_marker_event(self, coords):
        ...
        new_marker = self.map_widget.set_marker(
            coords[0],
            coords[1],
            text="Warehouse" + str(self.current_warehouse_counter),
            icon=self.warehouse_image,
            text_color="red",
        )
        r.warehouses["Warehouse" + str(self.current_warehouse_counter)] = (
            coords[0],
            coords[1],
        )
        self.marker_list_box.insert(
    ...
    def add_charging_point_marker_event(self, coords):
        ...
        new_marker = self.map_widget.set_marker(
            coords[0],
            coords[1],
            text="ChargingPoint" + str(self.current_charging_point_counter),
            icon=self.cp_image,
            text_color="green",
        )
        r.charging_points[
            "ChargingPoint" + str(self.current_charging_point_counter)
        ] = (coords[0], coords[1])
        self.marker_list_box.insert(
            tkinter.END,
            "ChargingPoint " + str(self.current_charging_point_counter) + " added",
        )
        self.marker_list_box.see(tkinter.END)
        self.current_charging_point_counter += 1
    ...
    def delete_last_marker(self, coords):
        if len(self.map_widget.canvas_marker_list) > 0:
            ...
            if "Warehouse" in self.last_element.text:
                self.current_warehouse_counter -= 1
                r.warehouses.popitem()
            if "Customer" in self.last_element.text:
                self.current_customer_counter -= 1
                r.delivery_locations.popitem()
                r.package_weights.popitem()
            if "Charging" in self.last_element.text:
                self.current_charging_point_counter -= 1
                r.charging_points.popitem()
            self.marker_list_box.insert(
    ...
    def clear_marker_list(self):
        ...
        self.current_charging_point_counter = 1
        r.warehouses.clear()
        r.package_weights.clear()
        r.delivery_locations.clear()
        r.selected_packages.clear()
        r.selected_packages_copy.clear()
        r.charging_points.clear()
        r.final_path.clear()
        r.distance_matrix.clear()
        ...
        print(
            self.current_charging_point_counter,
            self.current_customer_counter,
            self.current_warehouse_counter,
        )
    ...
    def connect_marker_products(self):
        ...
        marker_list_combined = dict(
            [(customer.text.split("\n")[0], customer) for customer in self.marker_list]
        )
        marker_list_combined.update(
            dict([(cp.text, cp) for cp in self.charging_points_marker_list])
        )
        marker_list_combined.update(
            dict([(w.text, w) for w in self.warehouse_marker_list])
        )
    ...
    def display_calculated_data(self):
        ...
        for i in self.final_path:
            p += i + "-->"
        self.marker_list_box.insert(tkinter.END, p[:-3])
